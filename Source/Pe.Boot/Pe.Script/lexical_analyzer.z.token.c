...
#define NONE_CHAR_SKIP_2 '\0', '\0'
#define NONE_KIND_SKIP_2 TOKEN_KIND_NONE, TOKEN_KIND_NONE
#define COMMENT_ALL_SKIP true, true, true, true

static MULTI_CHAR_TOKEN library__multi_character_tokens[] = {
    // +
    {
        .characters = { '+', '=', NONE_CHAR_SKIP_2, },
...
size_t read_multi_character_token(TOKEN_RESULT* token_result, const TEXT* source, size_t start_index, TOKEN_KIND last_token_kind, const SOURCE_POSITION* source_position, const PROJECT_SETTING* project_setting)
{
    ...
    const size_t characters_length = sizeof(characters) / sizeof(characters[0]);
    for (size_t i = 0; i < characters_length; i++) {
        characters[i] = get_relative_character(source, start_index, i);
    }

    for (size_t i = 0; i < sizeof(library__multi_character_tokens) / sizeof(library__multi_character_tokens[0]); i++) {
        const MULTI_CHAR_TOKEN* multi_token = library__multi_character_tokens + i;

        // 一文字目が合致しないのであればどう頑張っても比較する意味がない
        if (multi_token->characters[0] != characters[0]) {
            continue;
        }

        // 終端から最初に向かって戻していく
        for (size_t j = characters_length; j; j--) {
            size_t index = j - 1;
            if (!characters[index] || multi_token->kinds[index] == TOKEN_KIND_NONE) {
                continue;
            }

            bool all_characters_match = true;
            for (size_t k = 0; all_characters_match && k < j; k++) {
                all_characters_match &= characters[k] == multi_token->characters[k];
            }
            if (!all_characters_match) {
                continue;
            }

            if (!is_comment_token_kind(last_token_kind) || !multi_token->skip_comments[index]) {
                add_token_kind(&token_result->token, multi_token->kinds[index], source_position);
                return j;
            }
        }
    }
