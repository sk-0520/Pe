...
{
    const MEMORY_RESOURCE* memory_resource = syntax_set->script.memory_resource;

    for (size_t i = 0; i < syntax_set->length; i++) {
        for (size_t j = 0; j < syntax_set->defines[i].length; j++) {
            release_memory(syntax_set->defines[i].elements[j].data, memory_resource);
        }
        release_memory(syntax_set->defines[i].elements, memory_resource);
    }
    release_memory(syntax_set->defines, memory_resource);
    set_memory(syntax_set, sizeof(SYNTAX_SET), 0);
}
